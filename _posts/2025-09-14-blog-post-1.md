...
// add a picture to explain how the parallelization works

This is a bit harder to implement with a single queue (we would need 2 queues),
and coupled with the code complexity of a lock free queue, the implementation can get complex.   
Let's take a step back, and consider what we really need to keep track of for our BFS algorithm:
...
visited_nbrs(nbr_node) {
    ...
    if (state == NOT_VISITED) {
        if (atomic_cas(&global_visited[nbr_node], state, VISITED)
           path_length[nbr_node] = current_level + 1
    }
}
```
