...
So now, we have to suddenly keep track of multiple occurrences of a node across different levels. Having a single path length associated
with a node is not enough. How about the paths that we have to enumerate ? Because this query returns paths:
